...
echo "🔧 Fixing docker-compose context path (if needed)..."
# Fix FrontEnd -> Frontend path issue for frontend service
if [ "${serviceName}" = "frontend" ]; then
    if grep -q "context: ./FrontEnd" docker-compose.staging.yml 2>/dev/null; then
        echo "   Fixing context path from ./FrontEnd to ./Frontend..."
        sed -i 's|context: ./FrontEnd|context: ./Frontend|g' docker-compose.staging.yml
        echo "   ✅ Context path fixed"
    else
        echo "   ✅ Context path already correct"
    fi
fi
