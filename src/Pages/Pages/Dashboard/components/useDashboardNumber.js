...
const useDashboardNumber = () => {
  ...
  const totalSalesData = () => {
    ...
    try {
      // Use new totalSales field if available (from daily_sales_summary API)
      if (dashboardData?.totalSales !== undefined) {
        totalS = parseFloat(dashboardData.totalSales || 0);
      } else {
  ...
  const findSalesValue = (type) => {
    ...
    try {
      // Use new API data structure if available
      if (dashboardData?.breakdown) {
        if (type === "Store Sales") {
          // In-Store Sales
          totalS = parseFloat(dashboardData?.sales || dashboardData?.instoreSales || 0);
        } else {
          // Aggregator Sales
          totalS = parseFloat(dashboardData?.aggregatorTotal || dashboardData?.aggregatorSales || 0);
        }
      } else {
        // Fallback to old calculation logic
        if (type === "Store Sales") {
