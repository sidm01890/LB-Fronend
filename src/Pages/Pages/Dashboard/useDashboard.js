...
const useDashboard = () => {
  ...
  const getDashboard = async (params) => {
    ...
    setLoading(true);
    try {
      // Call new dashboard-sales API (from daily_sales_summary table)
      const dashboardSalesResponse = await requestCallPost(
        apiEndpoints.DASHBOARD_SALES,
        {
          startDate: params.startDate,
          endDate: params.endDate,
          stores: params.stores,
          cities: params.cities || [], // Extract cities from params if available
        }
      );

      // Process dashboard sales data
      if (dashboardSalesResponse.status && dashboardSalesResponse?.data?.data) {
        const salesData = dashboardSalesResponse.data.data;
        ...
        makeLog(
          ...
          {
            ...params,
            dashboard_type: "fetch_dashboard_sales",
          }
        );
        
        // Transform new API response to match existing Redux structure
        // For backward compatibility with existing components
        const transformedDashboardData = {
          sales: salesData.instoreSales || 0,
          aggregatorTotal: salesData.aggregatorSales || 0,
          totalSales: salesData.totalSales || 0,
          breakdown: salesData.breakdown || {},
        };
        
        const transformed3POData = {
          instoreTotal: salesData.instoreSales || 0,
          posSales: salesData.instoreSales || 0,
          aggregatorTotal: salesData.aggregatorSales || 0,
          threePOSales: salesData.aggregatorSales || 0,
          breakdown: salesData.breakdown || {},
        };
        
        dispatch(setDashboardData(transformedDashboardData));
        dispatch(setDashboard3POData(transformed3POData));
      } else {
        console.error("Dashboard sales API failed:", dashboardSalesResponse.message);
        // Fallback to old APIs if new API fails
      ...
      // Only set loading false after API completes and data is dispatched
